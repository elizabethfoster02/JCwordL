...
export class Cell extends React.Component<Props, State> {
  ...
  render() {
    const cellIndexClass = !!this.props.cellIndex
    ...
    const classes = classnames(
      ...
      cellIndexClass,
      {
        ...
        'bg-slate-400 text-white border-slate-400':
          this.props.status === 'absent' && this.state.revealPhase === 2,
        'bg-purple-500 text-white border-purple-500':
          this.props.status === 'correct' && this.state.revealPhase === 2,
        'bg-yellow-500 text-white border-yellow-500':
          this.props.status === 'present' && this.state.revealPhase === 2,
